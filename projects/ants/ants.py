...
class Ant(Insect):
    ...
    def add_to(self, place):                    
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            s = place.ant
            assert s.can_contain(self) or self.can_contain(s) , 'Two ants in {0}'.format(place)

            if s.can_contain(self):          
                s.store_ant(self)
            if self.can_contain(s):        
                self.store_ant(s)
                place.ant = self
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        def helper(self1,x):   
            if self1.is_hive:
                return None
            if self1.bees and self.lower_bound<=x<=self.upper_bound:
                return random_bee(self1.bees) # REPLACE THIS LINE
            else:
                return helper(self1.entrance,x+1)
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        print("DEBUG:",list(self.place.bees))
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.turns_to_chew !=0:
            ...
            if self.place.bees:
                s= random_bee(self.place.bees)
                s.reduce_health(s.health)
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while s:
            if s.ant:
                if s.ant.doubles == False:
                    s.ant.double()
                if s.ant.is_container == True and s.ant.ant_contained and s.ant.ant_contained.doubles== False:
                    s.ant.ant_contained.double()
            s = s.exit
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        s,distance={},0
        placeL=self.place
        while placeL and placeL.is_hive ==False: 
            if placeL.ant and placeL.ant is not self:
                s[placeL.ant]=distance

            if placeL.bees:
                for x in placeL.bees[:]:
                    s[x]=distance
            distance +=1 
            placeL =placeL.entrance
