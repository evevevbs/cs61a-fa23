...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        operators = expressions.rest.first
        

        
        operators = scheme_eval(operators,env)
        env.define(signature,operators)
        return signature
        # END PROBLEM 4
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        return do_define_form(Pair(signature.first,Pair(Pair('lambda',(Pair(signature.rest,expressions.rest))),nil)),env)
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    if first is False:
        return False
    return Unevaluated(Pair("and",expressions.rest),env)
...
def do_or_form(expressions, env):
    ...
    if isinstance(firsts,Pair):
            firsts= scheme_eval(firsts,env)
    if firsts is not False :
        return firsts
    if expressions.rest!=nil and expressions.rest.rest ==nil:
        return Unevaluated( expressions.rest.first,env)
    return Unevaluated(Pair("or",expressions.rest),env)
