...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in scheme_forms.SPECIAL_FORMS:
        ...
        if not isinstance(first,BuiltinProcedure):
            if not isinstance(first,LambdaProcedure):
                if not isinstance(first,MuProcedure):
                    first = env.lookup(first)
        if rest!=nil :
            rest= rest.map(lambda x : scheme_eval(x,env))
        # print("DEBUG:",first,repr(first),repr(rest)) 
        if isinstance(first,LambdaProcedure):
            return scheme_apply(first,rest,first.env)   
