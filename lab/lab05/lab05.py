...
def replace_loki_at_leaf(t, lokis_replacement):
    ...
    if len(t)<1:
        return []
    if label(t)=='loki'and len(t)==1:
        return [lokis_replacement]+[replace_loki_at_leaf(x,lokis_replacement) for x in branches(t)]

    return [label(t)]+[replace_loki_at_leaf(x,lokis_replacement) for x in branches(t)]
...
def distance(city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    s=(get_lat(city_a)-get_lat(city_b))**2+(get_lon(city_a)-get_lon(city_b))**2
    return sqrt(s)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    s=make_city(1,lat,lon)
    if distance(s,city_a)<=distance(s,city_b):
...
def dejavu(t, n):
    ...
    "*** YOUR CODE HERE ***"
    for x in branches(t):
        if len(x)==1 and x[0]==n-label(t):

            return True
        if dejavu(x,n-label(t)):
            return True
    return False
